...
const Feature: React.FC<FeatureProps> = ({ icon, title, description, className }) => (
  <div className={`card-premium p-6 hover-scale ${className}`}>
      ...
      <div className="py-16 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <Feature 
              icon={<div className="p-3 bg-blue-100 rounded-full w-fit"><Zap className="h-6 w-6 text-blue-600" /></div>}
              title="Smart Charging Management"
              description="Optimize charging schedules based on energy costs, vehicle usage patterns, and grid demand to minimize operational costs."
            />
            
            <Feature 
              icon={<div className="p-3 bg-green-100 rounded-full w-fit"><BarChart className="h-6 w-6 text-green-600" /></div>}
              title="Real-time Analytics"
              description="Comprehensive dashboard with real-time analytics on energy usage, costs, charging station status, and vehicle performance."
            />
            
            <Feature 
              icon={<div className="p-3 bg-indigo-100 rounded-full w-fit"><Lock className="h-6 w-6 text-indigo-600" /></div>}
              title="Security Framework"
              description="Enterprise-grade security to protect charging infrastructure, vehicle data, and user information from threats."
            />
            
            <Feature 
              icon={<div className="p-3 bg-purple-100 rounded-full w-fit"><Smartphone className="h-6 w-6 text-purple-600" /></div>}
              title="Mobile Applications"
              description="Driver apps for iOS and Android with route planning, charging station locations, and real-time vehicle status updates."
            />
            
            <Feature 
              icon={<div className="p-3 bg-amber-100 rounded-full w-fit"><Clock className="h-6 w-6 text-amber-600" /></div>}
              title="Predictive Maintenance"
              description="AI-powered predictive maintenance alerts to prevent downtime and extend the lifespan of vehicles and charging infrastructure."
            />
            
            <Feature 
              icon={<div className="p-3 bg-red-100 rounded-full w-fit"><Settings className="h-6 w-6 text-red-600" /></div>}
              title="Fleet Management"
              description="Comprehensive fleet management tools including vehicle assignment, driver scheduling, and performance tracking."
            />
          </div>
        </div>
      </div>
